...
class PostScheduler:
      ...
      def _setup_jobs(self):
                ...
                # Generate daily posts at 7 AM every day
          ...
          # Re-crawl websites at 6:30 AM to get fresh content before generation
                self.scheduler.add_job(
      ...
      def _generate_daily_drafts(self):
        ...
        for account in accounts:
                      try:
                                        ...
                                        logger.info(f"[Scheduler] Generated {len(posts)} posts for {account['business_name']}")

                      except Exception as e:
                                        logger.error(f"[Scheduler] Error generating posts for account {account['id']}: {e}")

                  logger.info(f"[Scheduler] Daily generation complete. Total posts created: {generated_count}")

    def _recrawl_all(self):
              """
                      Auto-called at 6:30 AM.
                              Re-crawls all linked business websites to refresh content.
                                      """
        ...
        for account in accounts:
                      try:
                                        ...
                                        logger.info(f"[Scheduler] Recrawled {account['business_url']}: {result.get('pages_count', 0)} pages")
except Exception as e:
